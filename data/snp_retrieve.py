...
def dbSNP_retrieve(chrom):
    ...
    for a in range(30, 51):
        ...
        search_term=f"{chrom}[Chromosome] AND" + \
                   f"(k[Allele] OR m[Allele] OR r[Allele] OR s[Allele] OR w[Allele] OR y[Allele]) " + \
                   f"AND {gmaf_range}[Global Minor Allele Frequency] " + \
                   "AND \"snv\"[SNP Class] " + \
                   "NOT \"merged rs\"[Filter]"
        handle = Entrez.esearch(db="snp", term=search_term, RetMax=200000)
        record = Entrez.read(handle)
        print(f"{record['Count']} matches found on Chromosome {chrom} for gmaf range {gmaf_range}")
